...
float lastSentTemp =
    -999.0f; // Valor inicial imposible para forzar envío primera vez
...
void handleSerialTemperatureInput() {
  while (Serial.available() > 0) {
    ...
    String tempStr = cmd;
    // Soporta tanto formato "T:XX.XX" como formato numerico normal "XX.XX"
    if (cmd.startsWith("T:") || cmd.startsWith("t:")) {
      tempStr = cmd.substring(2);
    }
    ...
    if (newTemp >= 0 && newTemp <= 50) {
      ...
      Serial.println("[TEMP] Error: rango debe estar entre 0 y 50°C. Ingrese "
                     "un valor valido (ej: 25.5)");
    }
  }
}
...
void handleConfigResponse(JsonVariant data) {
  ...
  // Enviar estado inicial del sensor inmediatamente tras conectar
  readAndSendSensors();
}
...
void readAndSendSensors() {
  ...
  if (currentTemp >= lastSentTemp - 0.05f &&
      currentTemp <= lastSentTemp + 0.05f) {
...
void setup() {
  ...
  Serial.println(
      "[INIT] Temperatura serial activa. Formato: T:XX.XX (rango 0-50)");
